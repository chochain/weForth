...
one: src/ceforth.cpp template/ceforth.html
	...
	$(EM) -o html/ceforth.html src/ceforth.cpp --shell-file template/ceforth.html -sEXPORTED_FUNCTIONS=_main,_forth,_vm_base,_vm_ss,_vm_ss_idx,_vm_dict_idx,_vm_dict,_vm_mem,_top -sEXPORTED_RUNTIME_METHODS=ccall,cwrap -O2

two: src/ceforth.cpp $(FLST)
	echo "WASM: eForth + one worker thread"
	cp $(FLST) ./html
	$(EM) -o html/weforth.js src/ceforth.cpp -sEXPORTED_FUNCTIONS=_main,_forth,_vm_base,_vm_ss,_vm_ss_idx,_vm_dict_idx,_vm_dict,_vm_mem,_top -sEXPORTED_RUNTIME_METHODS=ccall,cwrap -O2

debug: src/ceforth.cpp $(FLST)
	echo "WASM: create WASM objdump file"
	$(EM) -o html/ceforth.html src/ceforth.cpp --shell-file template/ceforth.html -sEXPORT_ALL=1 -sLINKABLE=1 -sEXPORTED_RUNTIME_METHODS=ccall,cwrap
	wasm-objdump -x html/ceforth.wasm > html/ceforth.wasm.txt

all: one two
	echo "cmd: python3 -m http.server to start local web server"
	echo "cmd: enter http://localhost:8000/html/ceforth.html or weforth.html to test"
