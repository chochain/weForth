...
: rnd_bdy ( id -- ) ds .id !           \ create a randam shape config with given id
  rnd SMAX * 1+ int ds .type !                \ shape
  rnd 0.5 + rnd 0.5 + rnd 0.5 + px 3! ;       \ x0, x1, x2
: rnd_geo                              \ random geometry (jump up from [0,0,0])
  0 0 0                         ds .pos 3!    \ position[x, y, z]
  rnd rnd rnd rnd 2PI *         ds .rot 4! ;  \ rotation[x, y, z, w]
: rnd_v 3 rx rnd 10 * 10 + 3 rx ds .lv  3! ;  \ linear velocity[x, y, z]
: rnd_w 1 rx rnd           1 rx ds .av  3! ;  \ angular velocity[x, y, z]
...
: color ds .type @ cells fg + @ ;      \ fetch shape color (defined by ds[1])
