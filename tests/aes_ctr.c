...
struct AES_ctx 
{
    ...
    uint8_t Iv[AES_NBLOCK];             // for CTR only
};
...
{
    uint8_t buffer[AES_NBLOCK];
  
    size_t i;
    int bi;
    for (i = 0, bi = AES_NBLOCK; i < length; ++i, ++bi)
    {
        if (bi == AES_NBLOCK) /* we need to regen xor compliment in buffer */
        {
      
            memcpy(buffer, ctx->Iv, AES_NBLOCK);
...
static int test_ctr()
{
    ...
    KeyExpansion(ctx.RoundKey, key);      // init ctx
