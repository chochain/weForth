...
typedef fop* FPTR;          ///< pointer to lambda
struct Code {
...
#else
typedef void (*FPTR)();     ///< function pointer WASM vtable[i]
struct Code {
    union {
        FPTR xt = 0;        ///< lambda pointer
        struct {            ///< a colon word
            U16 len:  14;   ///< reserved  (WASM 32-bit, use lower 14-bits)
            U16 immd: 1;    ///< immediate flag
            U16 def:  1;    ///< colon defined word
...
void forth(int i, char *cmd) {
    ...
    printf("&c=%8p, cp=%8p, xt=%8p def=%d, imm=%d name=%8p name=%s ",
           &c, cp, (FPTR)((uintptr_t)c.xt & 0x3fff), c.def, c.immd, c.name, c.name);
    for (int i=0; i < 8; i++) {
        printf("%02x ", *cp++);
    }
    printf("|xt=");
    cp = (uint8_t*)((uintptr_t)c.xt & 0x3fff);
    for (int i=0; i < 8; i++) {
        printf("%02x ", *cp++);
    }
    printf(" => ");
    ((FPTR)((uintptr_t)c.xt & 0x3fff))();
    printf("\n");
}
}
